...
A normal accepted-pass recheck follows Gate A -> Gate B -> Gate C -> Gate D.
Return to Stage 0 only when the accepted executable boundary or decomposition is
materially wrong. This sequence defines responsibilities but does not create
automatic transitions between gates.
...
### Gate A - Reconciliation And Planning
...
Perform concise reconciliation and planning for every recheck. The plan records:
...
Do not require a durable standalone plan document, intake, SHA, requirement
declaration, testing record, or mapping artifact merely because an older
workflow expected it. Historical plans remain provenance and may not resurrect
scope rejected by the corrected master.

Review the Gate A plan before implementation and correct material problems.
Route a wrong executable boundary to Stage 0 or program correction and

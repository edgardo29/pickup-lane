...
Exclude genuinely unrelated systems without manufacturing work for them. When a
material safeguard belongs to another layer or remains incomplete, identify the
owner or gap instead of pretending the page owns it.
...
7. Material gaps, exclusions, and later-owned safeguards include a reason.
...
### Step 4: Select Risk-Based Scenarios
...
Report validation performed, important results, and remaining gaps concisely.
...
Use the applicable items below as risk-discovery prompts. Do not require a
classification record or test for every listed item when it is unrelated or
adds no distinct protection. Report material missing safeguards, tests, rules,
or decisions.
...
For a complex or high-risk page change, a reviewable scenario inventory may be
useful in the task plan or completion report. Do not create permanent scenario
IDs or a dedicated testing document when a concise risk and validation summary
is sufficient.

The following format is optional when that additional detail is warranted:
...
A passing test must still prove intended behavior, an invariant, or a concrete
risk; a permanent mapping artifact is not required.
...
- Ask Codex to “add edge cases” without naming the material risks or scenarios.
...
Report the testing work concisely:

- intended behavior and material risks reviewed;
- important safeguards and scenarios proved;
- test layers and commands actually used, with observed results;
- material security, failure, or recovery behavior checked when applicable;
- remaining gaps, unresolved conflicts, and verification not run.

Do not claim complete coverage from a generic statement such as “tests were
added” or “coverage looks good.”

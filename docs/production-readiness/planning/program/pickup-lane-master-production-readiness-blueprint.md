...
- custom orchestration layered around the retained production-readiness stages and gates;
...
- focused, proportionate engineering planning;
- normal pytest;
...
Retain the established first-time implementation sequence:

```text
Stage 0 -> Gate A -> Gate B -> Gate C -> Gate D -> manual merge
```

The workflow organizes execution discipline. It does not define production-
readiness scope, create a launch requirement by itself, or override this
blueprint's NEED / DO NOT NEED decisions. Stages do not advance automatically;
each stage follows the current instruction and its edit, review, publication,
and stop boundaries.

1. **Stage 0 - Scope, reconciliation and decomposition**
   ...
   - decide whether the unit executes whole, decomposes into children, or needs
     a deferred follow-up. A no-decomposition decision still completes the
     Stage 0 responsibility.

2. **Gate A - Engineering planning and plan review**
   - create and review an engineering plan for the selected executable unit;
   - keep the plan concise when the work is straightforward;
   - define expected behavior, important invariants, implementation approach and validation;
   - do not create elaborate or reusable planning machinery merely because the
     old process expected it.

3. **Gate B - Implement and test**
...
4. **Gate C - Independent semantic review**
...
5. **Gate D - Git and PR finalization**
...
Codex does not require duplicated handoff bundles, custom repository-local agents, frozen planning SHAs, automatic agent-to-agent Gate orchestration or custom compliance-checker routing.

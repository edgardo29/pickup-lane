...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint(
            "attempt_type IN ('issue_credit_rejected', 'reverse_credit_rejected', 'suspend_user_rejected', 'delete_user_rejected')",
...
def downgrade() -> None:
    op.execute(
        "DROP TRIGGER trg_admin_rejected_attempts_immutable ON admin_rejected_attempts"
    )
    op.execute("DROP FUNCTION prevent_admin_rejected_attempts_mutation()")

...
def test_booking_status_history_create_get_list_and_update_reason(
    ...
    get_response = run_as_temporary_admin(
        client,
        lambda: client.get(f"/booking-status-history/{history['id']}"),
    )
    ...
    list_response = run_as_temporary_admin(
        client,
        lambda: client.get(f"/booking-status-history?booking_id={booking['id']}"),
    )
...
def test_booking_status_history_requires_admin_permission(client: TestClient):
    user, _venue, _game, booking = create_booking_status_history_setup(client)
    history = create_booking_status_history(
        client,
        booking["id"],
...
def test_booking_status_history_rejects_moderator(client: TestClient):
    user, _venue, _game, booking = create_booking_status_history_setup(client)
    history = create_booking_status_history(
        client,
        booking["id"],

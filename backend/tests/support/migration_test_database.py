...
def reset_migration_database(engine: Engine) -> None:
    ...
    with engine.begin() as connection:
        ...
        _drop_public_functions(connection)
...
def _drop_public_functions(connection: Connection) -> None:
    function_names = connection.execute(
        text(
            """
            SELECT format(
                '%I.%I(%s)',
    ...
    for function_name in function_names:
        connection.execute(text(f"DROP FUNCTION IF EXISTS {function_name} CASCADE"))

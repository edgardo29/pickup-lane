"""Payment mutation workflows for admin/support routes."""
...
def validate_payment_references(
    ...
    get_active_user_or_404(db, payment_data["payer_user_id"])

    if payment_data["booking_id"] is not None:
        db_booking = get_booking_or_404(db, payment_data["booking_id"])

        if db_booking.buyer_user_id != payment_data["payer_user_id"]:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Booking payments must use the booking buyer as payer_user_id.",
            )

    if payment_data["game_id"] is not None:
        get_active_game_or_404(db, payment_data["game_id"])


def payment_audit_metadata(
    payment: Payment,
    *,
    source: str,
    ...
    if before is not None:
        metadata["old_payment_status"] = before["payment_status"]
        metadata["new_payment_status"] = payment.payment_status
        metadata["before"] = before
        metadata["after"] = payment_audit_snapshot(payment)
    ...
    *,
    ...
    try:
        ...
        record_admin_action(
            ...
            target_payment_id=new_payment.id,
            metadata=payment_audit_metadata(
                new_payment,
...
def update_payment_record(
    ...
    *,
    ...
    before_snapshot = payment_audit_snapshot(db_payment)
    ...
    validate_payment_references(db, effective_payment_data)
    ...
    try:
        ...
        record_admin_action(
            ...
            target_payment_id=db_payment.id,
            metadata=payment_audit_metadata(
                db_payment,

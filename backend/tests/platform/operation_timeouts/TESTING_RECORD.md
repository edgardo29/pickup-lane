...
| `WS02-04C1-R8` | representative unknown-outcome side-effect ordering | pytest with fakes and PostgreSQL where persisted rejection effects matter | `test_timeout_side_effect_ordering_contract.py` | Confirms checkout timeout preserves only a recoverable local checkpoint without confirmation or success, Firebase deletion support metadata, saved-card unpersisted cleanup behavior, and no blind application retry without broad payment/account-system reimplementation. |
...
|---|---|---|---|
| Stripe checkout/payment timeout | committed pending local checkpoint only | no provider confirmation, definite local provider success, or local provider failure solely from timeout | local recovery identity survives and provider mutation is not replayed by C1 |

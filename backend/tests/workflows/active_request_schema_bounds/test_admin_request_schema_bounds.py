...
def test_admin_review_literals_and_text_bounds() -> None:
    ...
    for outcome in get_args(AdminReviewClosureOutcome):
        accepted = AdminReviewCaseClose(outcome=outcome, reason="x", **close_fields)
        assert accepted.outcome == outcome
    ...
    assert (
        AdminReviewCaseClose(
            outcome="no_action_needed", reason="x", **close_fields
        ).reason
        == "x"
    )
    assert (
        AdminReviewCaseClose(
            outcome="no_action_needed", reason="x" * 1000, **close_fields
        ).reason
        == "x" * 1000
    )
    _assert_rejected(

...
For backend test organization and environment safety, see
...
- Current tests for the same behavior, if any.
- Current shared fixtures and support utilities relevant to the scope.
...
- Which current tests already cover part of the behavior, if any.
...
bug. Create a test directory only when current coverage has a real ownership
need. Do not add placeholder domain, workflow, platform, migration, provider, or
support folders for future work.
...
- New or refactored tests must follow the behavior-ownership and support-file
  rules in this document and `backend/tests/README.md`.
...
obsolete helper infrastructure merely because older tests used it.
...
An explicit production-readiness task authorizes only the validation required by
...
- the intended behavior and material risks reviewed;
- the important scenarios and safeguards proved;
...
Use this checklist as risk-based review guidance. Apply the items that are
material to the behavior being changed; it is not a universal compliance record.

...
def build_seed_users(password: str) -> list[PlatformNoticeSeedUser]:
    return [
        PlatformNoticeSeedUser(
        ...
        PlatformNoticeSeedUser(
            email="platform.notice.user1@test.com",
            password=password,
            first_name="Alex",
            last_name="Rivera",
        ...
        PlatformNoticeSeedUser(
            email="platform.notice.user2@test.com",
            password=password,
            first_name="Jordan",
            last_name="Lee",
...
def upsert_firebase_user(seed_user: PlatformNoticeSeedUser) -> str:
    try:
...
def upsert_postgres_user(firebase_uid: str, seed_user: PlatformNoticeSeedUser) -> User:
...
def seed_platform_notice_test_users(password: str) -> None:
    initialize_firebase_admin()
    print("Platform Notice test users ready:")
    print("")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Create local Firebase/Auth DB users for Platform Notice manual QA."
        )
    )
    parser.add_argument(
        "--password",
...
def main() -> int:
    ...
    try:
        seed_platform_notice_test_users(args.password)

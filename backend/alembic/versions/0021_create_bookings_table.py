...
down_revision = '0020_venue_images'
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column('platform_fee_snapshot_cents', sa.Integer(), nullable=False, server_default=sa.text('0')),
        ...
        sa.CheckConstraint("payment_status IN ('not_required', 'unpaid', 'requires_action', 'processing', 'paid', 'failed', 'partially_refunded', 'refunded', 'credit_restored', 'disputed')", name='ck_bookings_payment_status'),
        sa.CheckConstraint('platform_fee_cents >= 0', name='ck_bookings_platform_fee_cents'),
        sa.CheckConstraint('platform_fee_snapshot_cents >= 0', name='ck_bookings_platform_fee_snapshot_cents'),
        sa.CheckConstraint('price_per_player_snapshot_cents >= 0', name='ck_bookings_price_per_player_snapshot_cents'),
        sa.CheckConstraint('subtotal_cents >= 0', name='ck_bookings_subtotal_cents'),
        sa.CheckConstraint('total_cents >= 0', name='ck_bookings_total_cents'),
        sa.CheckConstraint('total_cents = subtotal_cents + platform_fee_cents - discount_cents', name='ck_bookings_total_cents_formula'),
    ...
    op.create_index('ix_bookings_buyer_user_id', 'bookings', ['buyer_user_id'], unique=False)

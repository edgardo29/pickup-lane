...
CREATE_REVIEW_CASE_GUARD_SQL = """
CREATE OR REPLACE FUNCTION guard_admin_review_case_update()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
    IF NEW.case_type IS DISTINCT FROM OLD.case_type
       ...
       OR NEW.target_sub_post_id IS DISTINCT FROM OLD.target_sub_post_id THEN
        RAISE EXCEPTION 'review case identity is immutable';
    END IF;
    IF OLD.case_status = 'closed' AND NEW IS DISTINCT FROM OLD THEN
        RAISE EXCEPTION 'closed review case projection is immutable';
    END IF;
    RETURN NEW;
END;
$$;
CREATE TRIGGER trg_admin_review_cases_guard_update
BEFORE UPDATE ON admin_review_cases
FOR EACH ROW EXECUTE FUNCTION guard_admin_review_case_update();
"""
DROP_REVIEW_CASE_GUARD_SQL = (
    "DROP FUNCTION IF EXISTS guard_admin_review_case_update() CASCADE"
)
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint(
            "(case_status = 'open' AND closed_by_user_id IS NULL AND closure_outcome IS NULL AND closure_reason IS NULL AND closed_at IS NULL) OR (case_status = 'closed' AND closure_outcome IS NOT NULL AND closure_reason IS NOT NULL AND closed_at IS NOT NULL)",
        ...
        sa.CheckConstraint(
            "(case_type = 'community_game' AND target_game_id IS NOT NULL AND target_user_id IS NULL AND target_sub_post_id IS NULL AND target_sub_post_request_id IS NULL AND target_payment_id IS NULL AND target_financial_outcome_id IS NULL) OR (case_type = 'need_a_sub' AND target_sub_post_id IS NOT NULL AND target_user_id IS NULL AND target_game_id IS NULL AND target_sub_post_request_id IS NULL AND target_payment_id IS NULL AND target_financial_outcome_id IS NULL) OR (case_type NOT IN ('community_game', 'need_a_sub') AND (target_user_id IS NOT NULL OR target_game_id IS NOT NULL OR target_sub_post_id IS NOT NULL OR target_sub_post_request_id IS NOT NULL OR target_payment_id IS NOT NULL OR target_financial_outcome_id IS NOT NULL))",
    ...
    op.get_bind().exec_driver_sql(CREATE_REVIEW_CASE_GUARD_SQL)


def downgrade() -> None:
    op.get_bind().exec_driver_sql(DROP_REVIEW_CASE_GUARD_SQL)

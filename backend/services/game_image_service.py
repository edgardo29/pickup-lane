"""Game image scaffold read and admin mutation workflows."""
...
def get_game_image_or_404(db: Session, game_image_id: uuid.UUID) -> GameImage:
...
def get_public_game_image_or_404(
    db: Session,
    game_image_id: uuid.UUID,
) -> GameImage:
    db_game_image = get_game_image_or_404(db, game_image_id)

    if db_game_image.image_status != PUBLIC_IMAGE_STATUS:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Game image not found.",
        )
...
def reject_public_game_image_admin_filters(
    *,
    ...
    if image_status is not None and image_status != PUBLIC_IMAGE_STATUS:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Admin access required.",
        )
...
def get_public_game_image_record(
    db: Session,
    game_image_id: uuid.UUID,
) -> GameImage:
    return get_public_game_image_or_404(db, game_image_id)


def get_admin_game_image_record(
    db: Session,
    game_image_id: uuid.UUID,
) -> GameImage:
    return get_game_image_or_404(db, game_image_id)


def list_public_game_image_records(
    db: Session,
    *,
    ...
    statement = select(GameImage).where(
        GameImage.deleted_at.is_(None),
        GameImage.image_status == PUBLIC_IMAGE_STATUS,
    )
...
def list_admin_game_image_records(
    db: Session,
    *,
    game_id: uuid.UUID | None = None,
    uploaded_by_user_id: uuid.UUID | None = None,
